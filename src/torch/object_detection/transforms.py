        ...
        while True:
            ...
            for _ in range(self.trials):
                ...
                ious = torchvision.ops.boxes.box_iou(
                    boxes, torch.tensor([[left, top, right, bottom]], dtype=boxes.dtype,
                                        device=boxes.device)
                )
...
class RandomZoomOut(nn.Module):
    def __init__(
        self, fill: Optional[List[float]] = None, side_range: Tuple[float, float] = (1.0, 4.0),
        p: float = 0.5
    ):
        super().__init__()
        if fill is None:
...
class SimpleCopyPaste(torch.nn.Module):
    ...
    def forward(
        ...
        torch._assert(
            isinstance(images, (list, tuple)) and all(
                [isinstance(v, torch.Tensor) for v in images]),
        ...
        for image, target, paste_image, paste_target in zip(images, targets, images_rolled,
                                                            targets_rolled):
