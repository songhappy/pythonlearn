...
### NOTE AUC sci and auc using probabilities/rawPrediction as raw are the same
    ...
    for eta in [0.05, 0.1, 0.15, 0.2]:
        for max_depth in [4, 6, 8, 10]:
            for num_round in [100, 200, 400]:
                ...
                labels = [row.label for row in predicts.select("label").collect()]
                predictions = [row.prediction for row in predicts.select("prediction").collect()]
                probabilities = [row.probability[1] for row in predicts.select("probability").collect()]
                acc_sci = accuracy_score(labels, predictions)
                auc_sci = roc_auc_score(labels, probabilities)

                print(params)
                print("AUC: %.2f" % (auc * 100.0))
                print("AUC_Sci: %.2f" % (auc_sci * 100.0))
                print("Accuracy: %.2f" % (acc * 100.0))
                print("Accuracy_sci: %.2f" % (acc_sci * 100.0))
